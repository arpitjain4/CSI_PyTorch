...
class Trainer:
    ...
    def __init__(self, model, device, optimizer, criterion, scheduler, resume=None,
                 save_path='./checkpoints', print_freq=100, val_freq=2, test_freq=2):
    ...
    def loop(self, epochs, train_loader, val_loader, test_loader):
        ...
        for ep in range(self.cur_epoch, self.all_epoch + 1):
            ...
            #print(f"curr_epoch:{slep}")
            # conduct training, validation and test
            self.train_loss = self.train(train_loader)
            if ep % self.val_freq == 0:
                self.val_loss,val_rho, val_nmse = self.test(val_loader)
                # print(f"val_nmse={val_nmse}")
                # print(f"val_rho={val_rho}")

            if ep % self.test_freq == 0:
                self.test_loss, test_rho, test_nmse = self.test(test_loader)
            else:
                test_rho, test_nmse = None,None

            # conduct saving, visualization and log printing
            self._loop_postprocessing(test_rho,test_nmse)
    ...
    def val(self, val_loader):
        ...
        with torch.no_grad():
            ...
            return self.tester(val_loader, verbose=True)
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx,(sparse_ul,) in enumerate(data_loader):
            time_tmp = time.time()
            sparse_ul= sparse_ul.to(self.device)
            # sparse_dl= sparse_dl.to(self.device)
            sparse_pred_ul = self.model(sparse_ul)
            loss = self.criterion(sparse_ul,sparse_pred_ul)
            #loss = -(torch.mean(self.criterion(sparse_pred.reshape([sparse_pred.size(0),-1]), sparse_gt.reshape([sparse_gt.size(0),-1]))))**2
            ...
            #loss = self.criterion(sparse_gt,sparse_pred)
            #print(loss)
           ...
           # print(loss)
            ...
            if (batch_idx + 1) % self.print_freq == 0:
                logger.info(f'Epoch: [{self.cur_epoch}/{self.all_epoch}]'
                            ...
                            f'MSE loss: {iter_loss.avg:.5e} | '
                            f'time: {iter_time.avg:.3f}')

        mode = 'Train' if self.model.training else 'Val'
        logger.info(f'=> {mode}  Loss: {iter_loss.avg:.5e}\n')
...
class Tester:
    ...
    def __init__(self, model, device, criterion, print_freq=1):
        self.model = model
        self.device = device
        self.criterion = criterion
        self.print_freq = print_freq
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx,(sparse_ul,) in enumerate(data_loader):
           ...
           # print(loss)
            ...
            #print(loss)
            nmse = evaluate_nmse(sparse_pred_ul, sparse_ul)
            #raw_pred = post_processing(sparse_pred)
            rho = evaluate_rho(sparse_pred_ul,sparse_ul)
            #rho_gt = evaluate_rho(sparse_gt,raw_gt)

            # Log and visdom update
            iter_loss.update(loss)
            #print(f"{iter_loss.avg}")
            iter_rho.update(rho)
       ...
       # logger.info(f'=> Test rho:{iter_rho.avg:.3e}  NMSE: {iter_nmse.avg:.3e} mse = {mse}\n')
